...
一款 Android App：对手机正在播放的视频录屏（**默认只录系统音频**），抽取音频、转写为文本，再用大模型把转写稿总结要点、生成一篇结构化的文章总结。转写文本支持长按复制全文。

## 功能流水线

```
录屏(系统音频/可选麦克风) → 抽取音频(WAV) → 语音转文本(ASR) → AI 总结要点成文(LLM)
```
...
| `data.remote` | `Transcriber`（ASR）、`Summarizer`（LLM，长文分块提炼要点 map，再合并成总结 reduce，避免长视频超时） |
